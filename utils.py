...
def get_pop_scores(raw_scores, uncertainty, limit):
    pop_scores_CI = [[100-stats.percentileofscore(raw_scores, raw_score+uncertainty), 100-stats.percentileofscore(raw_scores, raw_score), 100-stats.percentileofscore(raw_scores, raw_score-uncertainty)] for raw_score in raw_scores[:limit]]
...
def get_answers(users):
    answers = {user_id: users[user_id]['answers'] for user_id in users.keys()}
    return answers
...
def get_top_scores(current_user, logged_users, limit, N_options=5):
    ...
    raw_scores_all, max_score, uncertainty = get_raw_scores(current, logged, N_options=N_options)

    argsort_idx = raw_scores_all.argsort()
    raw_scores_sorted = np.array(raw_scores_all)[argsort_idx]

    top_pop_scores_CI = get_pop_scores(raw_scores_sorted, uncertainty, limit=limit)
